...
> Статус: **Proposed** (требует решения; НЕ реализовано).
...
⏳ **Ожидает выбора пользователя/бизнеса.** До решения — A остаётся в
силе (round 2 в проде). Реализация B/C НЕ начинается без явного выбора и
бизнес-триггера.
